...
router.post("/:username/jobs/:id", ensureLoggedIn, isAdmin, async function (req, res, next) {
  try {
    ...
    const appl = await User.apply(req.body);
    return res.status(201).json({applied: appl });
  } catch (err) {
    return next(err);
  }
